...
const TOOLS = [
  ...
  { id:'crop',      icon:'⬚', label:'Crop PDF',           desc:'Visually crop margins with drag handles.',              color:'#3ecf8e', bg:'rgba(62,207,142,0.08)',  border:'rgba(62,207,142,0.25)',  category:'organize' },
  ...
  return (
    <div onClick={() => onClick(tool.id, tool.category)}
      ...
      style={{
        ...
        boxShadow: hovered ? '0 12px 36px rgba(0,0,0,0.25)' : 'none',
      }}>
      {tool.tag && (
      ...
      <div style={{ display:'flex', alignItems:'flex-start', gap:'14px' }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'5px' }}>
            <h3 style={{ fontSize:tool.featured?'17px':'14px', fontWeight:700, color:'var(--text)' }}>{tool.label}</h3>
            <ArrowRight size={13} color="var(--text3)"/>
          </div>
          <p style={{ fontSize:'12px', color:'var(--text2)', lineHeight:1.6 }}>{tool.desc}</p>
        </div>
      </div>
      {tool.featured && tool.perks && (
        <div style={{ display:'flex', flexWrap:'wrap', gap:'10px', marginTop:'18px', paddingTop:'16px', borderTop:'1px solid rgba(108,99,255,0.2)' }}>
          {tool.perks.map(p => (
            <span key={p} style={{ fontSize:'11px', color:'var(--accent2)', display:'flex', alignItems:'center', gap:'4px' }}>
...
function SectionHeading({ label }) {
  return (
    <div style={{ display:'flex', alignItems:'center', gap:'12px', marginBottom:'16px', marginTop:'8px' }}>
      <span style={{ fontSize:'12px', fontWeight:700, letterSpacing:'.08em', textTransform:'uppercase', color:'var(--text3)' }}>{label}</span>
      <div style={{ flex:1, height:'1px', background:'var(--border)' }}/>
    </div>
  )
}

export default function Dashboard({ onSelectTool, initialCategory='all', dark, onToggleTheme }) {
  ...
  const renderTools = () => {
    if (filtered) {
      if (!filtered.length) return (
        <div style={{ textAlign:'center', padding:'60px 0', color:'var(--text3)' }}>
          ...
          <p style={{ fontSize:'15px', fontWeight:600, color:'var(--text2)' }}>No tools found for "{search}"</p>
        </div>
      )
    ...
    if (cat==='convert') {
      ...
      return (<>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(270px,1fr))', gap:'14px', marginBottom:'32px' }}>{toTools.map(t=><ToolCard key={t.id} tool={t} onClick={onSelectTool}/>)}</div>
        <SectionHeading label="Convert from PDF"/>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(270px,1fr))', gap:'14px' }}>{fromTools.map(t=><ToolCard key={t.id} tool={t} onClick={onSelectTool}/>)}</div>
      </>)
    }
    const visible   = cat==='all' ? TOOLS : TOOLS.filter(t=>t.featured||t.category===cat)
    const displayed = (cat==='all'&&!showAll) ? visible.slice(0,8) : visible
    const hasMore   = cat==='all'&&!showAll&&visible.length>8
    return (<>
      ...
      {hasMore && (
        <div style={{ textAlign:'center', marginTop:'28px' }}>
          <button onClick={()=>setShowAll(true)}
            style={{ display:'inline-flex', alignItems:'center', gap:'8px', padding:'10px 28px', borderRadius:'999px', border:'1px solid var(--border)', background:'var(--bg2)', color:'var(--text3)', fontSize:'13px', fontWeight:500, cursor:'pointer', transition:'all 0.15s' }}
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--bg)' }}>
      <TopBar dark={dark} onToggleTheme={onToggleTheme} showSearch search={search} onSearch={setSearch}/>

      {/* Sticky category bar */}
      {!search && (
        <div style={{ position:'sticky', top:'52px', zIndex:40, background: dark?'rgba(13,13,20,0.95)':'rgba(245,245,250,0.95)', backdropFilter:'blur(12px)', borderBottom:'1px solid var(--border)', padding:'10px 24px', display:'flex', gap:'8px', justifyContent:'center', flexWrap:'wrap' }}>
          {CATEGORIES.map(c => (
            <button key={c.id} onClick={()=>setCat(c.id)} style={{
              fontSize:'13px', fontWeight:500, padding:'6px 18px', borderRadius:'999px',
              border:`1px solid ${cat===c.id?'var(--accent)':'var(--border)'}`,
              background: cat===c.id?'rgba(108,99,255,0.15)':'var(--bg2)',
      ...
      <main style={{ maxWidth:'1200px', margin:'0 auto', padding:'48px 24px' }}>
        {!search && (
          <div style={{ textAlign:'center', marginBottom:'48px' }}>
            ...
            <h1 style={{ fontSize:'54px', fontWeight:800, lineHeight:1.07, letterSpacing:'-0.03em', marginBottom:'16px', color:'var(--text)', fontFamily:"'Syne',sans-serif" }}>
            ...
            <p style={{ fontSize:'16px', color:'var(--text2)', maxWidth:'500px', margin:'0 auto', lineHeight:1.75 }}>
            ...
            <div style={{ display:'flex', gap:'48px', justifyContent:'center', marginTop:'60px', paddingTop:'40px', borderTop:'1px solid var(--border)', flexWrap:'wrap' }}>
              {[['29+','PDF Tools'],['100%','Free to use'],['50MB','Max file size'],['0','Data stored']].map(([n,l])=>(
                <div key={l} style={{ textAlign:'center' }}>
                  ...
                  <div style={{ fontSize:'13px', color:'var(--text2)', marginTop:'6px' }}>{l}</div>
                </div>
              ))}
            </div>

            <div style={{ marginTop:'60px', padding:'40px', background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'20px', textAlign:'center' }}>
              <h2 style={{ fontSize:'22px', fontWeight:800, color:'var(--text)', marginBottom:'10px' }}>Why Choose PDFForge?</h2>
              <p style={{ fontSize:'14px', color:'var(--text3)', marginBottom:'32px' }}>Built for privacy, speed, and simplicity.</p>
              <div style={{ display:'flex', gap:'32px', justifyContent:'center', flexWrap:'wrap' }}>
                {[['🔒','Private by Default','Files deleted immediately after processing.'],['⚡','Fast Processing','Server-side processing for large files.'],['🆓','Always Free','No accounts, no subscriptions, no tricks.'],['🌐','Works Everywhere','No software to install. Any modern browser.']].map(([icon,title,desc])=>(
      ...
      <footer style={{ borderTop:'1px solid var(--border)', padding:'28px 40px', background:'var(--bg)' }}>
        <div style={{ maxWidth:'1200px', margin:'0 auto', display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'16px' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
            ...
            <span style={{ fontSize:'14px', fontWeight:700, color:'var(--text)' }}>PDFForge</span>
            <span style={{ fontSize:'12px', color:'var(--text3)', marginLeft:'8px' }}>© 2025</span>
          </div>
          <div style={{ display:'flex', gap:'24px' }}>
            {['Privacy Policy','Terms of Use','Contact'].map(l=>(
              <a key={l} href="#" style={{ fontSize:'12px', color:'var(--text3)', textDecoration:'none' }}>{l}</a>
            ))}
          </div>
        </div>
      </footer>
