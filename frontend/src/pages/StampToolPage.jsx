...
// Helper function for page selection logic
...
export default function StampToolPage() {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      // Pointing to your failover architecture
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0b0b18', color: '#fff', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        {/* THUMBNAIL STRIP: Independent Scroll */}
        <div style={{ width: '150px', borderRight: '1px solid #252538', overflowY: 'auto', background: '#0f0f20', padding: '15px' }}>
          ...
          {thumbnails.map((t, i) => (
            <div key={i} onClick={() => setCurrentPage(i + 1)} style={{ marginBottom: '15px', cursor: 'pointer', textAlign: 'center', position: 'relative' }}>
              <img src={t.img} style={{ width: '100%', borderRadius: '4px', border: currentPage === i+1 ? '2px solid #6c63ff' : '1px solid #252538', opacity: isPageSelected(i+1, totalPages, range) ? 1 : 0.3 }} alt="" />
              {isPageSelected(i+1, totalPages, range) && <CheckCircle size={12} style={{ position:'absolute', top: 5, right: 5, color: '#6c63ff' }} />}
      ...
      {file && (
        <button onClick={handleApply} disabled={status === 'loading'} style={{ position: 'fixed', bottom: '30px', right: '30px', background: '#6c63ff', color: '#fff', border: 'none', padding: '12px 28px', borderRadius: '50px', fontWeight: 'bold', cursor: 'pointer', boxShadow: '0 10px 30px rgba(108,99,255,0.4)', display: 'flex', alignItems: 'center', gap: '10px' }}>
