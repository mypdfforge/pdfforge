...
import { ArrowLeft, Upload, Loader2, CheckCircle, AlertCircle, Info, X, Download, ChevronLeft, ChevronRight } from 'lucide-react'
import * as api from '../utils/api'
import axios from 'axios'

const CONFIGS = {
  merge:        { title:'Merge PDFs',        desc:'Combine multiple PDFs into one.',          multi:true,  guide:'Upload 2+ PDFs. They merge in the order shown.', fields:[], action:(f,v)=>api.mergePDFs(f), out:'merged.pdf', noPreview:true },
  ...
  extract:      { title:'Extract Pages',     desc:'Pull specific pages into a new PDF.',      guide:'Click page thumbnails to select, or type page numbers.',fields:[{id:'pages',label:'Pages',placeholder:'e.g. 1,3,5'}], useSelector:true, action:(f,v)=>{ const form=new FormData(); form.append('file',f[0]); form.append('pages',v.pages); return api._post('/tools/extract',form) }, out:'extracted.pdf' },
  rotate:       { title:'Rotate Pages',      desc:'Rotate PDF pages to any angle.',           guide:'Select pages and choose angle.',fields:[{id:'degrees',label:'Angle',type:'select',options:['90','180','270'],default:'90'},{id:'pages',label:'Pages',placeholder:'all',default:'all'}], useSelector:true, action:(f,v)=>api.rotatePDF(f[0],v.degrees||'90',v.pages||'all'), out:'rotated.pdf' },
  ...
  images:       { title:'PDF to Images',     desc:'Convert each page to PNG (ZIP).',         guide:'Each page becomes a PNG image.',fields:[], action:(f,v)=>api.pdfToImages(f[0]), out:'pdf_images.zip', mime:'application/zip', noPreview:true },
  'image-to-pdf':{ title:'Images to PDF',   desc:'Convert images to a PDF.',                guide:'Upload images — they become pages in order.',multi:true, accept:{'image/jpeg':['.jpg','.jpeg'],'image/png':['.png']}, fields:[], action:(f,v)=>api.imageToPDF(f), out:'from_images.pdf', noPreview:true },
  'pdf-to-word':{ title:'PDF to Word',       desc:'Convert to editable .docx.',              guide:'Upload PDF and download Word document.',fields:[], action:(f,v)=>api.pdfToWord(f[0]), out:'converted.docx', mime:'application/vnd.openxmlformats-officedocument.wordprocessingml.document', noPreview:true },
  ...
  'html-to-pdf':{ title:'HTML to PDF',       desc:'Convert HTML file to PDF.',               guide:'Upload your .html file.',accept:{'text/html':['.html','.htm']}, fields:[], action:(f,v)=>{ const form=new FormData(); form.append('file',f[0]); return api._post('/tools/html-to-pdf',form) }, out:'converted.pdf', noPreview:true },
  ...
  'pdf-to-xlsx':{ title:'PDF to Excel',      desc:'Extract content to spreadsheet.',         guide:'Text is placed into cells by position.',fields:[], action:(f,v)=>{ const form=new FormData(); form.append('file',f[0]); return api._post('/tools/pdf-to-xlsx',form) }, out:'converted.xlsx', mime:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', noPreview:true },
  'pdf-to-pdfa':{ title:'PDF to PDF/A',      desc:'Convert to archive-standard format.',     guide:'Creates a PDF/A-1b compliant file.',fields:[], action:(f,v)=>{ const form=new FormData(); form.append('file',f[0]); return api._post('/tools/pdf-to-pdfa',form) }, out:'converted_pdfa.pdf', noPreview:true },
  pagenums:     { title:'Add Page Numbers',  desc:'Add page numbers to every page.',          guide:'Choose position and click Add.',fields:[{id:'position',label:'Position',type:'select',options:['bottom-center','bottom-right','top-center','top-right'],default:'bottom-center'}], action:(f,v)=>api.pageNumbersPDF(f[0],v.position||'bottom-center'), out:'numbered.pdf' },
  ...
  protect:      { title:'Protect PDF',       desc:'Lock with a password.',                   guide:'Enter a strong password — it cannot be recovered.',fields:[{id:'password',label:'Password',placeholder:'Enter password',type:'password',tip:'Keep this safe'}], action:(f,v)=>{ const form=new FormData(); form.append('file',f[0]); form.append('password',v.password); return api._post('/tools/protect',form) }, out:'protected.pdf', noPreview:true },
...
function PageSidebar({ file, selectedPages, onSelectionChange }) {
  ...
  return (
    <div style={{ height:'100%', display:'flex', flexDirection:'column' }}>
      {/* Batch actions */}
      <div style={{ padding:'10px 12px', borderBottom:'1px solid var(--border)', display:'flex', gap:'5px', flexWrap:'wrap' }}>
        {[['All', selectAll],['None', selectNone],['Odd', selectOdd],['Even', selectEven]].map(([label, fn]) => (
          <button key={label} onClick={fn}
            style={{ fontSize:'11px', padding:'3px 9px', borderRadius:'6px', border:'1px solid var(--border)', background:'var(--bg3)', color:'var(--text2)', cursor:'pointer' }}>
            {label}
          </button>
        ))}
        {selectedPages.size > 0 && (
          <span style={{ fontSize:'11px', color:'var(--accent2)', marginLeft:'auto', alignSelf:'center' }}>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'10px', display:'flex', flexDirection:'column', gap:'8px' }}>
        {loading ? (
          <div style={{ textAlign:'center', padding:'20px', color:'var(--text3)', fontSize:'13px' }}>
            <Loader2 size={18} className="spin" style={{ display:'block', margin:'0 auto 8px' }}/>
            Loading pages…
          </div>
        ) : thumbs.map(t => {
          ...
          return (
            <div key={t.page} onClick={() => toggle(t.page)}
              style={{ cursor:'pointer', position:'relative', borderRadius:'8px', overflow:'hidden',
                border:`2px solid ${isSel ? 'var(--accent)' : 'var(--border)'}`,
                boxShadow: isSel ? '0 0 0 2px rgba(108,99,255,0.25)' : 'none',
                transition:'all 0.12s', background: isSel ? 'rgba(108,99,255,0.06)' : 'transparent' }}>
              <img src={t.img} alt={`Page ${t.page}`} style={{ width:'100%', display:'block' }}/>
              <div style={{ position:'absolute', top:'4px', right:'4px', width:'18px', height:'18px', borderRadius:'50%',
              ...
              <div style={{ textAlign:'center', fontSize:'10px', color:'var(--text3)', padding:'3px 0', background:'var(--bg2)' }}>p.{t.page}</div>
            </div>
          )
        })}
      </div>
...
/* ── PDF Preview Panel ──────────────────────────────────── */
function PreviewPanel({ file, currentPage, totalPages, onPageChange }) {
  const [thumbs, setThumbs] = useState([])
  const [loading, setLoading] = useState(false)
  ...
  if (!file) return (
    <div style={{ height:'100%', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', color:'var(--text3)', gap:'10px' }}>
  ...
  return (
    <div style={{ height:'100%', display:'flex', flexDirection:'column' }}>
      {/* Page nav */}
      {totalPages > 1 && (
        <div style={{ padding:'8px 16px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'center', justifyContent:'center', gap:'12px' }}>
          <button onClick={() => onPageChange(Math.max(1, currentPage-1))}
            disabled={currentPage <= 1}
            style={{ background:'none', border:'1px solid var(--border)', borderRadius:'6px', padding:'4px 8px', cursor:'pointer', color:'var(--text2)', opacity:currentPage<=1?0.4:1 }}>
            <ChevronLeft size={14}/>
          </button>
          ...
          <button onClick={() => onPageChange(Math.min(totalPages, currentPage+1))}
            ...
            style={{ background:'none', border:'1px solid var(--border)', borderRadius:'6px', padding:'4px 8px', cursor:'pointer', color:'var(--text2)', opacity:currentPage>=totalPages?0.4:1 }}>
            <ChevronRight size={14}/>
          </button>
        </div>
      )}

      <div style={{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', padding:'24px', overflowY:'auto', background:'#14141f' }}>
        {loading ? (
          ...
          <img src={thumb.img} alt={`Page ${currentPage}`}
            style={{ maxWidth:'100%', maxHeight:'100%', boxShadow:'0 4px 32px rgba(0,0,0,0.5)', borderRadius:'3px', background:'#fff' }}/>
        ) : (
...
export default function ToolPage({ toolId, onBack }) {
  ...
  // Sync selectedPages string into vals.pages
  useEffect(() => {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--bg)', display:'flex', flexDirection:'column' }}>
      ...
      {/* Header */}
      <header style={{ height:'52px', padding:'0 24px', display:'flex', alignItems:'center', gap:'12px', borderBottom:'1px solid var(--border)', background:'rgba(15,15,19,0.98)', backdropFilter:'blur(12px)', position:'sticky', top:0, zIndex:50, flexShrink:0 }}>
        <button onClick={onBack} className="btn btn-ghost" style={{ padding:'5px 10px', display:'flex', alignItems:'center', gap:'5px', fontSize:'13px', color:'var(--text2)', background:'none', border:'1px solid var(--border)', borderRadius:'8px', cursor:'pointer' }}>
      ...
      {showSplit ? (
        ...
        <div style={{ display:'flex', flex:1, height:'calc(100vh - 52px)', overflow:'hidden' }}>
          ...
          <div style={{ width:'140px', flexShrink:0, borderRight:'1px solid var(--border)', background:'var(--bg2)', overflow:'hidden', display:'flex', flexDirection:'column' }}>
            <div style={{ padding:'10px 12px', borderBottom:'1px solid var(--border)', fontSize:'11px', fontWeight:700, letterSpacing:'.06em', textTransform:'uppercase', color:'var(--text3)' }}>
              Pages
            </div>
          ...
          <div style={{ width:'320px', flexShrink:0, borderRight:'1px solid var(--border)', overflowY:'auto', padding:'20px' }}>
            ...
            {/* Guide */}
            {guide && cfg.guide && (
              <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'10px', padding:'12px 14px', marginBottom:'16px', display:'flex', alignItems:'flex-start', gap:'8px' }}>
                ...
                <button onClick={()=>setGuide(false)} style={{ background:'none', border:'none', color:'var(--text3)', cursor:'pointer', padding:0 }}><X size={12}/></button>
              </div>
            )}

            {/* Dropzone */}
            <div {...getRootProps()} style={{ background:isDragActive?'rgba(91,91,214,0.06)':'var(--bg2)', border:`1px dashed ${isDragActive?'var(--accent)':'var(--border)'}`, borderRadius:'10px', padding:'28px 16px', textAlign:'center', cursor:'pointer', marginBottom:'14px', transition:'all 0.15s' }}>
              <input {...getInputProps()}/>
              <Upload size={16} color="var(--accent2)" style={{ display:'block', margin:'0 auto 8px' }}/>
              {isDragActive
                ? <p style={{ color:'var(--accent2)', fontWeight:600, fontSize:'13px' }}>Drop it!</p>
                : <><p style={{ color:'var(--text)', fontWeight:500, fontSize:'13px' }}>Drop file or <span style={{ color:'var(--accent2)' }}>browse</span></p>
                   <p style={{ color:'var(--text3)', fontSize:'11px', marginTop:'3px' }}>{cfg.multi?'Multiple files':'PDF only'}</p></>}
            </div>

            {/* File list */}
            {files.length > 0 && (
              <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'10px', padding:'8px 12px', marginBottom:'12px' }}>
                {files.map((f,i) => (
                  <div key={i} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'5px 0', borderBottom:i<files.length-1?'1px solid var(--border)':'none' }}>
                    <span style={{ fontSize:'12px', color:'var(--text)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', flex:1 }}>📄 {f.name}</span>
                    <div style={{ display:'flex', gap:'8px', alignItems:'center', marginLeft:'8px', flexShrink:0 }}>
                      <span style={{ fontSize:'11px', color:'var(--text3)' }}>{(f.size/1024).toFixed(0)}KB</span>
                      <button onClick={()=>setFiles(p=>p.filter((_,j)=>j!==i))} style={{ background:'none', border:'none', color:'var(--text3)', cursor:'pointer', fontSize:'15px', lineHeight:1 }}>×</button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {cfg.useSelector && selectedPages.size > 0 && (
              <div style={{ background:'rgba(108,99,255,0.08)', border:'1px solid rgba(108,99,255,0.2)', borderRadius:'8px', padding:'8px 12px', marginBottom:'12px', fontSize:'12px', color:'var(--accent2)' }}>
            ...
            {inputFields.length > 0 && (
              <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'10px', padding:'14px', marginBottom:'12px', display:'flex', flexDirection:'column', gap:'12px' }}>
                  ...
                  <div key={field.id}>
                    ...
                    {field.type==='select' ? (
                      <select value={vals[field.id]||field.default||''} onChange={e=>setVals(v=>({...v,[field.id]:e.target.value}))}
                        style={{ width:'100%', background:'var(--bg3)', color:'var(--text)', border:'1px solid var(--border)', borderRadius:'6px', padding:'6px 10px', fontSize:'12px', outline:'none' }}>
                      ...
                      <input type={field.type||'text'} value={vals[field.id]||''} placeholder={field.placeholder}
                        ...
                        style={{ width:'100%', background:'var(--bg3)', color:'var(--text)', border:'1px solid var(--border)', borderRadius:'6px', padding:'6px 10px', fontSize:'12px', outline:'none', boxSizing:'border-box' }}/>
                    )}
                  </div>
                ))}
              </div>
            ...
            {/* Error */}
            {status === 'error' && (
              <div style={{ display:'flex', alignItems:'center', gap:'7px', background:'rgba(229,72,77,0.08)', border:'1px solid rgba(229,72,77,0.25)', borderRadius:'8px', padding:'9px 12px', marginBottom:'12px', fontSize:'12px', color:'var(--red)' }}>
            ...
            <button onClick={run} disabled={!files.length||status==='loading'}
              ...
              {status==='loading' ? <><Loader2 size={13} className="spin"/> Processing…</>
               : status==='done'  ? <><CheckCircle size={13}/> Downloaded!</>
               :                    <><Download size={13}/> {cfg.title}</>}
            </button>
          </div>
        ...
        /* ── Single-column layout (convert tools) ── */
        <div style={{ maxWidth:'580px', margin:'36px auto', padding:'0 24px' }}>

          {guide && cfg.guide && (
            <div style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'10px', padding:'12px 16px', marginBottom:'18px', display:'flex', alignItems:'flex-start', gap:'10px' }}>
              ...
              <button onClick={()=>setGuide(false)} style={{ background:'none', border:'none', color:'var(--text3)', cursor:'pointer', padding:0 }}><X size={13}/></button>
            </div>
          )}
