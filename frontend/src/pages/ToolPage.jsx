...
const CONFIGS = {
  ...
  compress:     { title:'Compress PDF',      desc:'Reduce file size.',                     guide:'Just upload — no settings needed.', fields:[], action:(f)=>{ const form=new FormData(); form.append('file',f[0]); return api._post('/tools/compress',form) }, out:'compressed.pdf', noPreview:true },
...
export default function ToolPage({ toolId, onBack, dark, onToggleTheme, onGoHome, showCategories, activeCategory, onCategoryChange, search, onSearch }) {
  ...
  // Wake Render backend on mount so it's warm when the user clicks run
  useEffect(() => { api.wakeRender() }, [])
